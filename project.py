...
image = cv2.imread("input/coin1.jpg") # read in image
...
sample_images_one = glob.glob("train/onepeso/*.png")
...
circles = cv2.HoughCircles(blurred, cv2.HOUGH_GRADIENT, dp = 2.2, minDist = 100, param1 = 200, param2 = 100, minRadius = 20, maxRadius = 120)
...
if materials[i] == "One":
    diameter = [x / biggest * 19.75 for x in diameter]
...
while i < len(diameter): # loop over diameters
    ...
    if m == "One": # if material is one peso
        t = "1 Peso"
